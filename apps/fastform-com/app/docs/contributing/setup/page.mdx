...
export const metadata = {
  title: "Fastform Development Setup: Complete Guide to Local Environment Configuration for Dev",
  description:
    "Step-by-step guide to setting up a development environment for Fastform. We officially support Gitpod and Github Codespaces for quick setup.",
};
...
We currently officially support the below methods to set up your development environment for Fastform.

<Note>
  Both the below cloud IDEs have a **generous free tier** to explore and develop! But make sure to not overuse
  the machines as Fastform will not be responsible for any charges incurred.
</Note>
...
This will open a Github VSCode Interface on the cloud for you. This setup will have the Fastform codebase and all the dependencies installed. Click the button below to configure your instance and open the project in Github Codespaces:
...
<Note>
  For a smooth experience, we suggest the above recommended methods. Assistance with setup issues on your
  local machine may be limited due to varying factors like OS and permissions.
</Note>
...
## Gitpod Guide
    ...
    **Initialization of Fastform:**
     - During the prebuilds phase, we initialize Fastform by performing the following tasks:
        ...
        4. Building the @fastform/js component.
     - When the workspace starts:
        1. Wait for the web and demo apps to launch on Gitpod. This automatically opens the `apps/demo/.env` file. Utilize dynamic localhost URLs (e.g., `localhost:3000` for signup and `localhost:8025` for email confirmation) to configure `NEXT_PUBLIC_FASTFORM_ENVIRONMENT_ID`. After creating your account and finding the `ID` in the URL at `localhost:3000`, replace `YOUR_ENVIRONMENT_ID` in the `.env` file located in `app/demo`.

    **Web Component Initialization:**
     - we initialize the @fastform/web component during prebuilds. This involves:
        1. Installing build dependencies for the `@fastform/web#go` task from turbo.json in prebuilds to save time.
     ...
     - When the workspace starts:
        ...
        3. Starting the `@fastform/web` dev environment.
    ...
    **Demo Component Initialization:**
     - Similar to the web component, the demo component is also initialized during prebuilds. This includes:
        1. Installing build dependencies for the `fastform/demo#go` task from turbo.json in prebuilds to save time.
        2. Caching hits and replaying builds from the `@fastform/js` component.
     - When the workspace starts:
        1. Initializing environment variables.
        2. Replaces `NEXT_PUBLIC_FASTFORM_API_HOST` to take in Gitpod URL's ports when running on VSCode browser.
        3. Starting the `@fastform/demo` dev environment.
...
- **Port 3001**:
  ...
  - **Service**: Fastform website
  - **Description**: This port hosts the [Fastform](https://getfastform.com) website, which contains documents, pricing, blogs, best practices, and concierge service.

- **Port 3002**:

  - **Service**: Fastform In-product Form Demo App
  - **Description**: This app helps you test your in-app forms. You can create and test user actions, create and update user attributes, etc.
...
<Col>
<CodeGroup title="Run the entire Fastform Stack">
...
```bash
@fastform/web:dev:   ▲ Next.js 13.5.6
@fastform/web:dev:   - Local:        http://localhost:3000
@fastform/web:dev:   - Environments: .env
@fastform/web:dev:   - Experiments (use at your own risk):
@fastform/web:dev:      · serverActions
@fastform/web:dev:
@fastform/web:dev:  ✓ Ready in 9.4s
```

</CodeGroup>
</Col>

8. Right next to the Terminal, you will see a **Ports** tab, click on it to see the ports and their respective URLs. Now access the Forwarded Address for port 3000 and you should be able to visit your Fastform App!
