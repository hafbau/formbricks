...
- Single-use links (or one-time / disposable links) are URLs that grant access to a form only once.

- The primary purpose of single-use links is to assure that no respondent submits a form twice.

## Using Single-Use Links with Fastform

Using single-use links with Fastform is quite straight-forward:

1. In the form settings, toggle "Single Use Link" on:

   <Image
     src={Settings}
     alt="Single use form settings"
     quality="100"
     className="max-w-full rounded-lg sm:max-w-3xl"
   />

2. When you publish your form, the following modal will open:
   <Image
     src={ShareModal}
...
You can customize the 'link used' messaging in the Form Editor settings:
