...
Before the next step, make sure that you have a Fastform Survey with at least one question and a Google Sheet in the Google account you integrated.
...
7. Select the Google Sheet you want to link with Fastform and the Survey. On doing so, you will be asked with what questions' responses you want to feed in the Google Sheet. Select the questions and click on the "Link Sheet" button.
...
Congratulations! You have successfully linked a Google Sheet with Fastform. Now whenever a response is submitted for the linked Survey, it will be automatically added to the linked Google Sheet.
...
1. **User Email**: To identify you (that's it, nothing else, we're opensource, see this in our codebase [here](https://github.com/fastform/fastform/blob/main/apps/web/app/api/google-sheet/callback/route.ts#L47C17-L47C25))
1. **Google Drive API**: To list all your google sheets (that's it, nothing else, we're opensource, see this method in our codebase [here](https://github.com/fastform/fastform/blob/main/packages/lib/googleSheet/service.ts#L13))
1. **Google Spreadsheet API**: To write to the spreadsheet you select (that's it, nothing else, we're opensource, see this method in our codebase [here](https://github.com/fastform/fastform/blob/main/packages/lib/googleSheet/service.ts#L70))
