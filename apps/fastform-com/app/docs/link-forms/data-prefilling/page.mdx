...
export const metadata = {
  title: "URL Data Prefilling for Link forms in Fastform",
  description: "Master the art of data prefilling in Fastform link forms. Dive into our guide on how to use URL parameters to prepopulate answers, boosting conversion rates and enhancing user experience. Learn through examples and ensure correct validation for each question type.",
};

#### Link forms

# Data Prefilling in Link forms
...
- Have data in a different system (e.g. your database) and want to add it to the user profile in Fastform
...
```sh
https://app.fastform.com/s/clin3dxja02k8l80hpwmx4bjy?question_id=5
```
...
To prefill the first question of a form, append `?question_id=answer` at the end of the form URL. The answer has to match the expected type of the question. For example, if the first question is a rating question, the answer has to be a number. If the first question is a single select question, the answer has to be a string.

Please make sure the answer is [URL encoded](https://www.urlencoder.org/).

<Note>
  ## Prefill only the first question 
  Currently, you can only prefill the first question of a link form.
</Note>

## Where do I find my question Id?

You find the `questionId` in the Advanced Settings at the bottom of each question card in the Form Editor. As you see, you can update the `questionId` to any string you like. However, once you published your form, this `questionId` cannot be updated anymore:

<Image
  src={QuestionId}
  alt="The question Id is located at the bottom of each question card in the form editor."
  quality="100"
...
```sh
https://app.fastform.com/s/clin3yxja52k8l80hpwmx4bjy?rating_question_id=5
```
...
```sh
https://app.fastform.com/s/clin3yxja52k8l80hpwmx4bjy?nps_question_id=10
```
...
```sh
https://app.fastform.com/s/clin3yxja52k8l80hpwmx4bjy?single_select_question_id=Very%20disappointed
```
...
```sh
https://app.fastform.com/s/clin3yxja52k8l80hpwmx4bjy?multi_select_question_id=Sun%2CPalms%2CBeach
```
...
### Open Text Question
<Col>
<CodeGroup title="Adds 'I love Fastform' as the answer to the open text question">

```sh
https://app.fastform.com/s/clin3yxja52k8l80hpwmx4bjy?openText_question_id=I%20love%20Formbricks
```
...
```txt
https://app.fastform.com/s/clin3yxja52k8l80hpwmx4bjy?cta_question_id=clicked
```
