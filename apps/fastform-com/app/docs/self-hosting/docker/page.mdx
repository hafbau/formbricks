export const metadata = {
  title: "Guide to Deploying Fastform Using Docker",
  description:
    "Step-by-step tutorial on how to effortlessly set up and run Fastform via Docker. Explore the quick deployment process with Docker-Compose, learn how to update Fastform, and troubleshoot common issues. Ideal for those looking for a hassle-free Fastform experience",
};
...
Quickly set up and start using Fastform without getting into the technicalities of the build process. You'll be using an image that we've already built for you. The pre-built image is ready-to-run, and it only requires minimal configuration on your part. This approach is perfect for getting a functional instance of Fastform up and running with minimal hassle. It's as easy as downloading the Docker image and firing up the container.

This is ideal for those who are testing Fastform or want to run it with minimal to no modifications. For this we use the [public Docker image](https://hub.docker.com/r/fastform/fastform) and a simple docker-compose file.
...
1.  **Create a New Directory for Fastform**

Open a terminal and create a new directory for Fastform, then navigate into this new directory:

<Col>
   <CodeGroup title="Create and cd into the new directory">

    ```bash
    mkdir fastform-quickstart && cd fastform-quickstart
    ```
...
Download the docker-compose file directly from the Fastform repository:
...
You're now ready to start the Fastform Docker setup. The following command will start Fastform together with a postgreSQL database using Docker Compose:
...
6. **Visit Fastform in Your Browser**

   After starting the Docker setup, visit http://localhost:3000 in your browser to interact with the Fastform application. The first time you access this page, you'll be greeted by a setup wizard. Follow the prompts to define your first user and get started.

## Updating Fastform

1.  Stop the Fastform stack
...
```bash
[+] Running 9/16
⠹ fastform 15 layers [⣿⣤⣿⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀] 29.78MB/47.76MB Pulling                                                           13.3s
...
[+] Running 2/2
✔ Container fastform-quickstart-postgres-1    Created                                                                       0.0s
✔ Container fastform-quickstart-fastform-1  Created                                                                       0.0s
```
...
```bash
fastform-quickstart-fastform-1  | All migrations have been successfully applied.
fastform-quickstart-fastform-1  |
fastform-quickstart-fastform-1  | - info Loaded env from /home/nextjs/apps/web/.env
fastform-quickstart-fastform-1  | Listening on port 3000 url: http://<random-string>:3000
```
...
To edit any of the variables that start with `NEXT_PUBLIC_`, you need to rebuild the Fastform Docker Image!
...
| DATABASE_URL                | Database URL with credentials.                                                        | required                                                | `postgresql://postgres:postgres@postgres:5432/fastform?schema=public` |
| NEXTAUTH_SECRET             | Secret for NextAuth, used for session signing and encryption.                         | required                                                | (Generated by the user)                                               |
| NEXTAUTH_URL                | Location of the auth server. By default, this is the Fastform docker instance itself. | required                                                | `http://localhost:3000`                                               |
...
| INSTANCE_ID                 | Instance ID for Fastform Cloud to be sent to Telemetry.                               | optional                                                |                                                                       |
| INTERNAL_SECRET             | Internal Secret (Currently we overwrite the value with a random value).               | optional                                                |                                                                       |
| IS_FASTFORM_CLOUD           | Uses Fastform Cloud if set to `1`                                                     | optional                                                |                                                                       |
...
| Variable                                       | Description                                                     | Required | Default |
| ---------------------------------------------- | --------------------------------------------------------------- | -------- | ------- |
| NEXT_PUBLIC_FASTFORM_API_HOST                  | Host for the Fastform API.                                      | optional |         |
| NEXT_PUBLIC_FASTFORM_ENVIRONMENT_ID            | If you already have a preset environment ID of the environment. | optional |         |
| NEXT_PUBLIC_FASTFORM_ONBOARDING_FORM_ID        | If you already have an onboarding form ID to show new users.    | optional |         |
...
| NEXT_PUBLIC_FASTFORM_COM_ENVIRONMENT_ID        | Environment ID for Fastform (if you already have one)           | optional |         |
| NEXT_PUBLIC_FASTFORM_COM_DOCS_FEEDBACK_FORM_ID | Form ID for the feedback form on the docs site.                 | optional |         |
| NEXT_PUBLIC_FASTFORM_COM_API_HOST              | Host for the Fastform API.                                      | optional |         |

Still facing issues? [Join our Discord!](https://getfastform.com/discord) and we'd be glad to assist you!
