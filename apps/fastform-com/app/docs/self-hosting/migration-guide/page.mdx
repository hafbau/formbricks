export const metadata = {
  title: "Migrating Fastform to v1.1",
  description:
    "Fastform v1.1 comes with an amazing set of features including the ability to define most environment variables at runtime itself! No need to build the image again! This guide will help you migrate your existing Fastform instance to v1.1",
};
...
Fastform v1.2 ships a lot of features targeting our Link forms. We have also improved our security posture to be as robust as ever. However, it also comes with a few breaking changes specifically with the environment variables. This guide will help you migrate your existing Fastform instance to v1.2 without any hassles or build errors.
...
| SHORT_URL_BASE       | false    | `<your-short-base-url>` | Needed if you want to enable shorter links for Link forms |

### Deprecated / Removed Environment Variables

| Environment Variable | Comments                                                                |
| -------------------- | ----------------------------------------------------------------------- |
| form_BASE_URL        | The WEBAPP_URL is now used to determine the form base url in all places |

## v1.0 -> v1.1

Fastform v1.1 includes a lot of new features and improvements. However, it also comes with a few breaking changes specifically with the environment variables. This guide will help you migrate your existing Fastform instance to v1.1 without losing any data.
...
| **NEXT*PUBLIC***IS_FASTFORM_CLOUD           | IS_FASTFORM_CLOUD           |
| **NEXT*PUBLIC***form_BASE_URL               | form_BASE_URL               |
...
x-environment: &environment
  environment:
    # The url of your Fastform instance used in the admin panel
    WEBAPP_URL:

    # PostgreSQL DB for Fastform to connect to
    DATABASE_URL: "postgresql://postgres:postgres@postgres:5432/fastform?schema=public"
    ...
    # Uncomment the below and set a value to have your own Terms Page URL on the auth and the forms page
    # TERMS_URL:

    # Uncomment the below and set a value to have your own Imprint Page URL on the auth and the forms page
    # IMPRINT_URL:
...
Did we miss something? Are you still facing issues migrating your app? [Join our Discord!](https://getfastform.com/discord) We'd be happy to help!
```
