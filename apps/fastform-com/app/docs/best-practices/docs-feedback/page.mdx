...
export default function DocsFeedback() {
  ...
  if (
    !process.env.NEXT_PUBLIC_FORMBRICKS_COM_DOCS_FEEDBACK_form_ID ||
...
export const handleFeedbackSubmit = async (YesNo, pageUrl) => {
  ...
  const payload = {
    ...
    formId: process.env.NEXT_PUBLIC_FORMBRICKS_COM_DOCS_FEEDBACK_form_ID, // For testing, replace this with the form ID of your form (more info below)
  };

  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_FORMBRICKS_COM_API_HOST}/api/v1/client/environments/${process.env.NEXT_PUBLIC_FORMBRICKS_COM_ENVIRONMENT_ID}/responses`, // For testing, replace this with the API host and environemnt ID of your Development environment on app.fastform.com
  };
      {
        method: "POST",
        headers: {
