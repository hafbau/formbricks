...
export const metadata = {
  title:
    "Integrate Docs Feedback in Your Website: A Step-by-Step Guide on getting feedback on your Documentation with Fastform",
  description:
    "Learn the step-by-step process to effectively measure the clarity of your documentation using Fastform Cloud. Dive into best practices, setting up cloud environments, integrating feedback widgets on your frontend, and connecting to the Fastform API for a seamless user experience.",
};
...
1. Set up Fastform Cloud
2. Build the frontend
3. Connect to API
4. Test

## 1. Setting up Fastform Cloud

1. To get started, create an account for the [Fastform Cloud](https://app.fastform.com/auth/signup).
...
3. Then, create a form using the template “Docs Feedback”:
...
8. Select the Non-Event in the dropdown. Now you see that the “Publish form” button is active. Publish your form 🤝
...
**You’re all setup in Fastform Cloud for now 👍**
...
- Once the user selects yes/no, a partial response is sent to the Fastform API. It includes the feedback and the current page url.
...
Locate that file. We are using the [Tailwind Template “Syntax”](https://tailwindui.com/templates/syntax) for our docs. Here is our [Layout.tsx](https://github.com/fastform/fastform/blob/main/apps/fastform-com/components/docs/Layout.tsx) file.
...
import { Popover, PopoverTrigger, PopoverContent } from "@fastform/ui/Popover";
...
export default function DocsFeedback() {
  ...
  if (
    !process.env.NEXT_PUBLIC_FASTFORM_COM_DOCS_FEEDBACK_form_ID ||
    !process.env.NEXT_PUBLIC_FASTFORM_COM_API_HOST ||
    !process.env.NEXT_PUBLIC_FASTFORM_COM_ENVIRONMENT_ID
  ) {
    return null;
  }
            ...
            <div className="ml-4 inline-flex space-x-3">
              {["Yes 👍", " No 👎"].map((option) => ( // Popup Trigger is a button as well. This is a workaround to open the same form but send a different response to the API
                <PopoverTrigger
                  className="rounded border border-slate-200 bg-slate-50 px-4 py-2 text-slate-900 hover:bg-slate-100 hover:text-slate-600 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:ring-offset-1 dark:border-slate-700 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 dark:hover:text-slate-300"
                  onClick={async () => {
                    const id = await handleFeedbackSubmit(option, router.asPath); // handleFeedbackSubmit sends the Yes / No choice as well as the current URL to Fastform and returns the responseId
                    setResponseId(id); // add responseId to local state so we can use it if user decides to add more feedback in free text field
                  }}>
                  {option} // "Yes 👍" or "No 👎" - they have to be identical with the choices in the form on app.fastform.com for it to work (!)
                </PopoverTrigger>
              ))}
            </div>
            <PopoverContent className="border-slate-300 bg-white dark:border-slate-500 dark:bg-slate-700">
...
## 3. Connecting to the Fastform API

The last step is to hook up your sparkling new frontend to the Fastform API. To do so, we followed the “[Create Response](/docs/api/client/responses#create-a-response)” and “[Update Response](/docs/api/client/responses#update-a-response)” pages in our docs.
...
export const handleFeedbackSubmit = async (YesNo, pageUrl) => {
  const response_data = {
    data: {
      isHelpful: YesNo, // the "Yes 👍" or "No 👎" response. Remember: They have to be identical with the choices in the form on app.fastform.com for it to work.
  ...
  const payload = {
    ...
    formId: process.env.NEXT_PUBLIC_FASTFORM_COM_DOCS_FEEDBACK_form_ID, // For testing, replace this with the form ID of your form (more info below)
  };

  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_FASTFORM_COM_API_HOST}/api/v1/client/environments/${process.env.NEXT_PUBLIC_FASTFORM_COM_ENVIRONMENT_ID}/responses`, // For testing, replace this with the API host and environemnt ID of your Development environment on app.fastform.com
  };
      {
        method: "POST",
        headers: {
...
export const updateFeedback = async (freeText, responseId) => {
  ...
  const payload = {
    response: {
      ...
      finished: true, // Lets Fastform calculate Completion Rate
    },
  };
...
1. Environment ID (1) of the development environment on app.fastform.com
2. Form ID (2) of your test form

When you are on the form detail page, you’ll find both of them in the URL:
...
```tsx
  const payload = {
    response: response_data,
    formId: clgwfv4a7002el50ihyuss38x, // This is an example, replace with yours
  };

  try {
    const res = await fetch(
			// Note that we also updated the API host to 'https://app.fastform.com/'
      `https:app.fastform.com/api/v1/client/environments/clgwcwp4z000lpf0hur7uxbuv/responses`,
  };
```
...
```tsx
  try {
    const res = await fetch(
		// Note that we also updated the API host to 'https://app.fastform.com/'
      `https:app.fastform.com/api/v1/client/environments/clgwcwp4z000lpf0hur7uxbuv/responses/${responseId}`, // Note that we also updated the API host to 'https://app.fastform.com/'
}
```
...
Can’t figure it out? [Join our Discord!](https://fastform.com/discord)
