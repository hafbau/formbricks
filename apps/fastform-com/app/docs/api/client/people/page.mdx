...
export const metadata = {
  title: "Fastform Public Client API Guide: Manage Users",
  description:
    "Dive deep into Fastform' Public Client API designed for customisation. This comprehensive guide provides detailed instructions on creating and updating users to help in user identification.",
};
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X POST \
    'https://app.fastform.com/api/v1/client/<environment-id>/people' \
...
<Row>
  ...
  <Col sticky>
    ...
    curl -X POST \
    --location \
    'https://app.fastform.com/api/v1/client/<environment-id>/people/<user-id>'
    -H 'Content-Type: application/json' \
    -d '{
        "attributes":{
            "welcome_to":"fastform"
