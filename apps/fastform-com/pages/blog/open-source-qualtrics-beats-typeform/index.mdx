...
import FastformSneak from "./fastform-sneak.png";
...
<Image src={Wrestling} alt="Why we do it" className="w-full rounded-lg" />

_In September, we kicked it off with a Typeform open-source alternative. As we build and learn, our focus is shifting. We talk about how we look at form and form tools today, why experience management not only matters for enterprise and why the endgame looks a lot more like open-source Qualtrics. Qualtrics? What happened to Typeform?_

Let’s dive in 👇

## How it started

<Image src={Snoopforms} alt="How it began - snoopforms landing page" className="w-full rounded-lg" />
...
<Image src={Tellform} alt="Tellform" className="w-full rounded-lg" />
<Image src={Ohmyform} alt="Ohmyform" className="w-full rounded-lg" />
<Image src={Tripetto} alt="Tripetto" className="w-full rounded-lg" />
...
We wrote about that in more detail in [Open source forms will save the world](https://getfastform.com/blog/open-source-forms-will-save-the-world) but in short, there are three benefits for customers: Data privacy when self-hosting, full customizability & extendability and a weaker lock-in.
...
<Image src={TypeformValue} alt="Why we do it" className="w-full rounded-lg" />

The average conversion rate of an emailed out Typeform is 5%. In-app forms for example average out at [30% so 6x better.](https://news.ycombinator.com/item?id=30377349) Typeforms are the most pretty forms out there and likely to convert better than a Google Form but are by no means the best way to get insights from your audience.
...
And yet, the yawning gap for an open-source form and form tool kept us engaged. So we dug deeper. Before we zoom in, let’s zoom out for context:
...
This is where Fastform comes into play.

## Take what’s good, update what’s old

When we redesigned Fastform, we came up with a set of characteristics the solution needs to pack:
...
- Has virality built in (form links)
- Allows simple generic form building (good for OS traction and sales argument to replace Typeform / formMonkey / other form tool with self-hosted alternative)

In a direct comparison to Qualtrics, this is how we position Fastform:
...
And we learned that the best developer experience is no developer experience. In other words: The less time a dev spends on implementing Fastform, the better.

So we built that. We built a visual form creator, event tracking, user attribute management and an analysis dashboard built for SaaS products. Users can implement Fastform via NPM or by copying a snippet into the head of their products HTML. That's it.

With the new Fastform you can **trigger any micro-form on any event in your web app.** Very soon you can **filter based on user attributes to form exactly and only the segment of your user** base which offers signal instead of noise.

You can create events either in your code base or with our No Code Event creator. Yep, even a non-technical person can copy the Fastform code snippet into the `<head>` tag and use our UI to create events to granularly trigger forms in the web app. **And they can do so in minutes.**

There is so much you can do with it, here is a sneakpeek:

<Image src={FastformSneak} alt="Fastform Sneak Preview" className="w-full rounded-lg" />

What all of this looks like in action we’ll share in a product walkthrough video soon. We are super excited to get the new Fastform into your hands asap.
